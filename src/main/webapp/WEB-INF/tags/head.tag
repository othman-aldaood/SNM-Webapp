...
<head>
    ...
    <%-- PWA: manifest + icons so the app can be installed to a phone/desktop home screen --%>
    <link rel="manifest" href="/snm-webapp/manifest.json">
    <meta name="theme-color" content="#2563eb">
    <link rel="apple-touch-icon" href="/snm-webapp/icons/apple-touch-icon.png">
    <meta name="apple-mobile-web-app-capable" content="yes">
    <meta name="apple-mobile-web-app-title" content="SharkNet">
    ...
    <%-- PWA: register the service worker so static assets are cached for offline access --%>
    <script>
        if ('serviceWorker' in navigator) {
            window.addEventListener('load', function () {
                navigator.serviceWorker.register('/snm-webapp/service-worker.js')
                    .then(function (reg) {
